    ...
    def take_turn(self):
        ...
        if state in ['agressive']:
            ...
            if _dist <= 1:
                ...
                monster.fighter.state = 'agressive'
...
def place_thing(thing,_wid=-1,has_stairs=True,sparse=2):
    ...
    while not done:
    		...
    		if map[_x][_y].terrain in acceptable and map[_x-_wid][_y-_wid].terrain in acceptable:
    			if  map[_x+_wid][_y+_wid].terrain in acceptable: 
    			  if map[_x-_wid][_y+_wid].terrain in acceptable:
    				if map[_x+_wid][_y-_wid].terrain not in acceptable:
						print 'x{},y{} unnaceptable, sent them home.'.format(_x,_y)
        			else:
        				done = True
...
def make_world_map(grassness=20,start=False):
    ...
    if start:
    	...
    	map[player.x-1][player.y-2] = Tile(True,terrain='-')
    	map[player.x-3][player.y-2] = Tile(True,terrain='-')
...
def place_homes(room):
    x = libtcod.random_get_int(0, room.x1+5, room.x2-5)
...
def place_objects(room):
    ...
    monster_chances['kobold mid_level'] = from_dungeon_level([[85, 2], [95, 5], [0, 7]])
    monster_chances['kobold high_level'] = from_dungeon_level([[15, 3], [30, 5], [60, 7]])
...
def addMonster(name,x=-1,y=-1,state='aggressive'):
	...
	if name.split()[0] in ['low_level','kobold']:
	...
	elif name.split()[0] in ['monster_mid_level']:
	...
	elif name.split()[0] in ['monster_high_level']:
	    ...
	    ai_component = BasicMonster()
	    monster = Object(x, y, '@', 'farmer giles', libtcod.darker_red, blocks=True, fighter=fighter_component, ai=ai_component)
