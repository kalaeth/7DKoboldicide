...
class PlayerFighter:
    ...
    def power(self):  #return actual power, by summing up the bonuses from all equipped items
        ...
        bonus += sum(skill.power for skill in self.skills)
...
def place_thing(thing,_wid=-1,avoid=(0,0),has_stairs=True,sparse=2):
    if _wid == -1:
        _wid = MAP_WIDTH / 10
    global map, stairs
...
def addMonster(name,x=-1,y=-1):
	...
	if name == 'monster_low_level':
	    ...
	    fighter_component = Fighter(hp=4, defense=0, power=2, xp=35, death_function=monster_death)
	    ai_component = BasicMonster()
	    monster = Object(x, y, 'k', 'monster_low_level', libtcod.light_red, blocks=True, fighter=fighter_component, ai=ai_component)
	elif name == 'monster_mid_level':
	    #create a monster_mid_level
	    fighter_component = Fighter(hp=5, defense=2, power=4, xp=65, death_function=monster_death)
	    ai_component = BasicMonster()
	    monster = Object(x, y, 'k', 'monster_mid_level', libtcod.red, blocks=True, fighter=fighter_component, ai=ai_component)
	elif name == 'monster_high_level':
	    #create a high_level_monster
	    fighter_component = Fighter(hp=6, defense=4, power=6, xp=85, death_function=monster_death)
	    ai_component = BasicMonster()
	    monster = Object(x, y, 'k', 'monster_high_level', libtcod.darker_red, blocks=True, fighter=fighter_component, ai=ai_component)
	else:
...
def render_all():
    ...
    libtcod.console_print_ex(panel_v, 0, 3, libtcod.BKGND_NONE, libtcod.LEFT, 'floor '+str(dungeon_level))
    libtcod.console_print_ex(panel_v, 0, 4, libtcod.BKGND_NONE, libtcod.LEFT, player.fighter.orientation)
