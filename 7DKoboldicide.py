#!/usr/bin/python
#
# 7DKoboldicider - 2017 7DRL Challenge
        ...
        if x is not None:
            if len(self.message) > 1:
                for _x in range(x, x+len(self.message)):
                    libtcod.console_put_char(con, _x, y, ' ', libtcod.BKGND_NONE)
                render_all()
            else:
...
class Fighter:
    ...
    def talk(self,text):
        ...
        notifications.append(Notif(text,5,x,y))
...
def place_thing(thing,_wid=-1,has_stairs=True,sparse=2):
    ...
    print '[{}]x,y({}-{},{}-{})[{}][{}][{}]'.format(thing,_x-_wid,_x+_wid,_y-_wid,_y+_wid,map[_x][_y].terrain,map[_x-_wid][_y-_wid].terrain,map[_x+_wid][_y+_wid].terrain)
    while (map[_x][_y].terrain not in acceptable and map[_x-_wid][_y-_wid].terrain not in acceptable and map[_x+_wid][_y+_wid] not in acceptable ):
    	print '[{}]x,y({}-{},{}-{})[{}][{}][{}]'.format(thing,_x-_wid,_x+_wid,_y-_wid,_y+_wid,map[_x][_y].terrain,map[_x-_wid][_y-_wid].terrain,map[_x+_wid][_y+_wid].terrain)
    	_x = random.randint(startC,MAP_WIDTH-_wid)
        _y = random.randint(startC,MAP_HEIGHT-_wid)

    print "center @[{},{}]".format(_x,_y)
...
def make_world_map(grassness=20,start=False):
    ...
    objects.append(stairs)
    c = place_thing(CHAR_DIRT,7, True, 6);
    objects.append(stairs)
    player.x, player.y = c
    if start:
    	map[player.x-2][player.y-2] = Tile(True,terrain='_')
    	map[player.x-1][player.y-2] = Tile(True,terrain='_')
    	map[player.x-3][player.y-2] = Tile(True,terrain='_')
...
def place_objects(room):
    ...
    monster_chances = {}
    monster_chances['monster_low_level'] = 80  #only enemy up to level 3.
    monster_chances['monster_mid_level'] = from_dungeon_level([[85, 2], [95, 5], [0, 7]])
    monster_chances['monster_high_level'] = from_dungeon_level([[15, 3], [30, 5], [60, 7]])
    ...
    item_chances['short sword'] = from_dungeon_level([[25, 3]])
    item_chances['long sword'] =  from_dungeon_level([[25, 6]])
    item_chances['leather armour'] =   from_dungeon_level([[10, 2]])
...
# addItem('leather armour',self.x,self.y)
def addItem(name,x=-1,y=-1):
...
def addMonster(name,x=-1,y=-1):

	if x == -1:
		x = random.randint(0, MAP_WIDTH-1)
	if y == -1:
		y = random.randint(0,MAP_HEIGHT-1)
	if name == 'monster_low_level':
	    #create a low level monster
	    fighter_component = Fighter(hp=2, defense=0, power=1, xp=35, death_function=monster_death)
	    ai_component = BasicMonster()
	    monster = Object(x, y, 'k', 'kobold low_level', libtcod.light_red, blocks=True, fighter=fighter_component, ai=ai_component)
	elif name == 'monster_mid_level':
	    #create a monster_mid_level
	    fighter_component = Fighter(hp=3, defense=2, power=2, xp=65, death_function=monster_death)
	    ai_component = BasicMonster()
	    monster = Object(x, y, 'k', 'kobold mid_level', libtcod.red, blocks=True, fighter=fighter_component, ai=ai_component)
	elif name == 'monster_high_level':
	    #create a high_level_monster
	    fighter_component = Fighter(hp=4, defense=4, power=3, xp=85, death_function=monster_death)
	    ai_component = BasicMonster()
	    monster = Object(x, y, 'k', 'kobold high_level', libtcod.darker_red, blocks=True, fighter=fighter_component, ai=ai_component)
	else:
		fighter_component = Fighter(hp=1, defense=1, power=1, xp=3, death_function=monster_death_no_loot)
		ai_component = BasicMonster()
...
def render_all():
    ...
    render_bar(CAMERA_WIDTH+1, 1, BAR_WIDTH, '+', player.fighter.hp, player.fighter.max_hp,
               libtcod.light_red, libtcod.darker_red)
    render_bar(CAMERA_WIDTH+1, 2, BAR_WIDTH, '$ ', player.fighter.purse, 200,
               libtcod.darker_green, libtcod.black)

    #libtcod.console_print_ex(panel, )
    #libtcod.console_print_ex(panel_v, 1)
    libtcod.console_print_ex(panel_v, 1, 3, libtcod.BKGND_NONE, libtcod.LEFT, 'floor  ['+str(dungeon_level)+']')
    libtcod.console_print_ex(panel_v, 1, 4, libtcod.BKGND_NONE, libtcod.LEFT, 'compass['+player.fighter.orientation+']')
    ...
    if game_state == 'playing':
        ...
        if key.vk == libtcod.KEY_UP or key.vk == libtcod.KEY_KP8 or chr(key.c) == 'w':
            ...
            if key_char in ['q','Q'] or key.vk == libtcod.KEY_KP1:
            	...
            	if curr_weap is not None and curr_weap.owner.name.split(' ')[1] in ['spear','bow','boomerang']:
	                    ...
	                    for _y in range(player.y+1, MAP_HEIGHT):
	                        ...
	                        for f in fs:
	                            if f.x == _x and f.y == _y:
	                                ...
	                                break
	                        notifications.append(Notif('|',3,player.x,_y))
	                elif player.fighter.orientation == WEST or _break:
	                    for _x in range(player.x-1,0,-1):
	                        if _break or map[_x][player.y].blocked:
	                            break
	                        for f in fs:
	                            if f.x == _x and f.y == _y:
	                            	fit = f
            ...
            if key_char in ['f','F'] or key.vk == libtcod.KEY_KP9:
            	curr_weap = get_equipped_in_slot('right hand')
            	if curr_weap is not None and curr_weap.owner.name.split(' ')[1] in ['sword','axe']:
	                if player.fighter.orientation == EAST:
	                    ...
	                    if not player_move_or_attack(0, 1,False) and not player_move_or_attack(+1, 1,False) and not player_move_or_attack(-1, 1,False):
	                        ...
	                        swing_sword(player.x+1,player.y+1)
	                        return 'attack'
            	elif curr_weap is not None and curr_weap.owner.name.split(' ')[1] in ['spear']:
					if player.fighter.orientation == EAST:
					    notifications.append(Notif('-',3,player.x+1,player.y))
...
def load_floor(floorname='world'):
    global map, objects, stairs, dungeon_level,player
    for ob in objects:
        if ob.name == "dot":
            new_obj = ob
            break
    ...
    file = shelve.open(floorname)
    #file = shelve.open(floorname, 'r')
...
def new_game():
    ...
    addMonster('monster_low_level',player.x,player.y+5)
...
def next_level(dl,terrain=CHAR_MOUNTAIN,up=False):
    ...
    if not up:
    	...
    	if dl == 0:
            ...
            save_floor('dungeon_{}'.format(dungeon_level))
            dungeon_level+=1
            make_map(terrain)  #create a fresh new level!
            initialize_fov()
    else:
        ...
        if dl == 1:
            ...
            dungeon_level -= 1
            load_floor('dungeon_{}'.format(dungeon_level))
            initialize_fov()
    save_game()
    if lair:
    	add_dragon()
